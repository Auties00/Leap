...
public class ChaCha20Engine extends TlsCipherEngine.Stream {
    ...
    static {
        ...
        for(var i = 0; i < tauSigma.length; i++) {
            tauSigma[i] = BufferUtils.readLittleEndianInt32(chars, i * 4);
        }
        TAU_SIGMA = tauSigma;
    }
    ...
    public void init(boolean forEncryption, byte[] key) {
        ...
        for (var i = 0; i < 4; i++) {
            engineState[4 + i] = BufferUtils.readLittleEndianInt32(key, i * 4);
        }
        for (var i = 0; i < 4; i++) {
            engineState[8 + i] = BufferUtils.readLittleEndianInt32(key, key.length - 16 + i * 4);
        }
    }
