...
import it.auties.leap.SocketClient.SecurityLayer;
...
final class HttpDecoder {
    ...
    private final SecurityLayer socket;
    ...
    HttpDecoder(SecurityLayer socket) {
        this.socket = socket;
        this.headers = null;
        this.body = null;
    ...
    <T> CompletableFuture<Result<T>> readResponse(URI uri, HttpResponse.Converter<T> converter) {
    ...
    sealed interface Result<T> {
        record Response<T>(int statusCode, boolean closeConnection, HttpResponse<T> data) implements Result<T> {

        }

        record Redirect<T>(URI to) implements Result<T> {

        }
    }

    private <T> CompletableFuture<Result<T>> handleResponse(URI uri, HttpResponse.Converter<T> converter) {
        ...
        if(contentLength() == 0) {
            return CompletableFuture.completedFuture(new Result.Response<>(statusCode, closeConnection, converter.empty(statusCode)));
        }

        return (contentLength() == -1 ? readChunkedResponse() : readFullResponse())
                .thenApplyAsync(response -> new Result.Response<>(statusCode, closeConnection, converter.of(statusCode, response)));
    }
