...
import it.auties.leap.tls.extension.TlsExtension.Model.Dependencies;
...
public final class TlsExtensionsProcessor {
    public static Result process(TlsConfig config) {
        // Find all compatible extensions
        var compatibleExtensions = config.extensions()
                .stream()
                .filter(extension -> extension.versions().contains(config.version()))
                .toList();

        // Make sure there are no conflicts
        var dependenciesTree = new HashMap<>();
        ...
        var processedExtensions = new ArrayList<>();
        var processedExtensionsLength = 0;
        var supportedGroups = TlsSupportedGroup.supportedGroups();
        var explicitVersionsSupport = false;
        var greaseSupport = false;
    ...
    private static int getRoundIndex(Dependencies.Some some) {
        var roundIndex = 0;
    ...
    private Optional<? extends TlsExtension.Concrete> configureExtension(TlsExtension extension) {
        return switch (extension) {
            ...
            case TlsExtension.Model<?, ?> modelExtension -> switch (modelExtension) {
                ...
                case KeyShareExtensionModel keyShareExtensionModel -> {
                    ...
                    var config = new KeyShareExtensionModel.Config(keyPair.publicKey(), keyPair.group());
    ...
    public record Result(
            Map<Class<? extends TlsExtension>, Dependencies> dependenciesTree,
            List<List<TlsExtension>> rounds,
            List<TlsExtension.Concrete> processedExtensions,
            int processedExtensionsLength,
            List<TlsSupportedGroup> supportedGroups,
            boolean extendedMasterSecret,
            boolean explicitVersionsSupport,
            boolean greaseSupport
    ) {
