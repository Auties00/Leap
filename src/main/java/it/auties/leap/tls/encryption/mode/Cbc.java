...
public final class Cbc extends TlsEncryption {
    ...
    public Cbc(TlsVersion version, Type type, TlsAuthenticator authenticator, TlsSessionKeys sessionKeys, TlsEngineMode mode) {
    ...
    private int addPadding(ByteBuffer bb) {
        ...
        var newlen = len + 1;
        if ((newlen % Cbc.BLOCK_SIZE) != 0) {
            newlen += Cbc.BLOCK_SIZE - 1;
            newlen -= newlen % Cbc.BLOCK_SIZE;
        }
    ...
    private void removePadding(ByteBuffer output) throws BadPaddingException {
        ...
        if (version == TlsVersion.SSL30) {
            if (padValue > Cbc.BLOCK_SIZE) {
                throw new BadPaddingException("Padding length (" + padValue + ") of SSLv3 message should not be bigger than the block size (" + Cbc.BLOCK_SIZE + ")");
            }
        }else {
