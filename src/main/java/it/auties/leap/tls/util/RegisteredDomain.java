    ...
    class DomainName {
        ...
        private static class Rules {
            ...
            private static class RuleSet {
                ...
                boolean hasExceptions = false;
                private static final Type[] AUTHS =
                    Type.values();
                ...
                void addRule(int auth, String rule) {
                    if (rule.startsWith("!")) {
                        rules.add(new Rule(rule.substring(1), DomainName.Rule.Type.EXCEPTION,
                                           AUTHS[auth]));
                        hasExceptions = true;
                    } else if (rule.startsWith("*.") &&
                               rule.lastIndexOf('*') == 0) {
                        rules.add(new Rule(rule.substring(2), DomainName.Rule.Type.WILDCARD,
                                           AUTHS[auth]));
                    } else if (rule.indexOf('*') == -1) {
                        // a "normal" label
                        rules.add(new Rule(rule, DomainName.Rule.Type.NORMAL, AUTHS[auth]));
                    } else {
