...
public final class GcmCipher extends TlsCipher.Block {
    ...
    public void encrypt(byte contentType, ByteBuffer input, ByteBuffer output) {
        var sn = authenticator.sequenceNumber();
        byte[] iv;
        boolean tls13 = authenticator.version() == TlsVersion.TLS13 || authenticator.version() == TlsVersion.DTLS13;
        if(tls13) {
            iv = fixedIv.clone();
            int offset = iv.length - sn.length;
            ...
            System.arraycopy(sn, 0, iv, fixedIv.length, sn.length);
            output.put(output.position() - sn.length, sn);
        }
        ...
        if(tls13) {
            sequence = sn;
            length = input.remaining() + tagLength();
        }else {
            sequence = null;
            length = input.remaining() - (forEncryption ? 0 : tagLength());
        }

        var aad = authenticator.createAuthenticationBlock(contentType, length, sequence);
        processAADBytes(aad, 0, aad.length);

        System.out.println("Message out!\nNonce: %s\nAAD: %s".formatted(java.util.Arrays.toString(iv), java.util.Arrays.toString(aad)));
        ...
        output.position(output.position() - (!tls13 ? dynamicIvLength(): 0));
        output.limit(output.position() + (!tls13 ? dynamicIvLength(): 0) + resultLen);
    }
    ...
    public ByteBuffer decrypt(TlsContext context, TlsMessageMetadata metadata, ByteBuffer input) {
        ...
        int length;
        if(authenticator.version() == TlsVersion.TLS13 || authenticator.version() == TlsVersion.DTLS13) {
            length = input.remaining();
        }else {
            length = input.remaining() - (forEncryption ? 0 : tagLength());
        }

        var aad = authenticator.createAuthenticationBlock(metadata.contentType().id(), length, null);
        processAADBytes(aad, 0, aad.length);
