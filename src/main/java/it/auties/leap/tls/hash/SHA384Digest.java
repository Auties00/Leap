...
public final class SHA384Digest implements TlsHashType {
    ...
    private SHA384Digest(SHA384Digest other) {
        this.h1 = other.h1;
        this.h2 = other.h2;
        this.h3 = other.h3;
        this.h4 = other.h4;
    ...
    public int digest(byte[] output, int offset, int length, boolean reset) {
        if (reset) {
            ...
            if(remainingLength > 0) {
                longToBigEndian(h1, output, offset, Math.min(remainingLength, BLOCK_LENGTH));
                remainingLength -= BLOCK_LENGTH;
            }

            if(remainingLength > 0) {
                longToBigEndian(h2, output, offset + 8, Math.min(remainingLength, BLOCK_LENGTH));
                remainingLength -= BLOCK_LENGTH;
            }

            if(remainingLength > 0) {
                longToBigEndian(h3, output, offset + 16, Math.min(remainingLength, BLOCK_LENGTH));
                remainingLength -= BLOCK_LENGTH;
            }

            if(remainingLength > 0) {
                longToBigEndian(h4, output, offset + 24, Math.min(remainingLength, BLOCK_LENGTH));
                remainingLength -= BLOCK_LENGTH;
            }

            if(remainingLength > 0) {
                longToBigEndian(h5, output, offset + 32, Math.min(remainingLength, BLOCK_LENGTH));
                remainingLength -= BLOCK_LENGTH;
            }

            if(remainingLength > 0) {
                longToBigEndian(h6, output, offset + 40, Math.min(remainingLength, BLOCK_LENGTH));
            }
            ...
            var digest = new SHA384Digest(this);
    ...
    private void processWord(byte[] in, int inOff) {
        w[wOff] = bigEndianToLong(in, inOff);
