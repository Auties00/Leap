...
public final class MD5 extends TlsHash {
    ...
    public int digest(byte[] output, int offset, int length, boolean reset) {
        if (reset) {
            ...
            if(remaining > 0) {
                Pack.intToLittleEndian(h1, output, offset, Math.min(remaining, BLOCK_LENGTH));
                remaining -= BLOCK_LENGTH;
            }

            if(remaining > 0) {
                Pack.intToLittleEndian(h2, output, offset + 4, Math.min(remaining, BLOCK_LENGTH));
                remaining -= BLOCK_LENGTH;
            }

            if(remaining > 0) {
                Pack.intToLittleEndian(h3, output, offset + 8, Math.min(remaining, BLOCK_LENGTH));
                remaining -= BLOCK_LENGTH;
            }

            if(remaining > 0) {
                Pack.intToLittleEndian(h4, output, offset + 12, Math.min(remaining, BLOCK_LENGTH));
            }
