...
public final class SHA256 extends TlsHash {
    ...
    public int digest(byte[] output, int offset, int length, boolean reset) {
        if (reset) {
            ...
            if(remaining > 0) {
                Pack.intToBigEndian(h1, output, offset, Math.min(remaining, BLOCK_LENGTH));
                remaining -= BLOCK_LENGTH;
            }

            if(remaining > 0) {
                Pack.intToBigEndian(h2, output, offset + 4, Math.min(remaining, BLOCK_LENGTH));
                remaining -= BLOCK_LENGTH;
            }

            if(remaining > 0) {
                Pack.intToBigEndian(h3, output, offset + 8, Math.min(remaining, BLOCK_LENGTH));
                remaining -= BLOCK_LENGTH;
            }

            if(remaining > 0) {
                Pack.intToBigEndian(h4, output, offset + 12, Math.min(remaining, BLOCK_LENGTH));
                remaining -= BLOCK_LENGTH;
            }

            if(remaining > 0) {
                Pack.intToBigEndian(h5, output, offset + 16, Math.min(remaining, BLOCK_LENGTH));
                remaining -= BLOCK_LENGTH;
            }

            if(remaining > 0) {
                Pack.intToBigEndian(h6, output, offset + 20, Math.min(remaining, BLOCK_LENGTH));
                remaining -= BLOCK_LENGTH;
            }

            if(remaining > 0) {
                Pack.intToBigEndian(h7, output, offset + 24, Math.min(remaining, BLOCK_LENGTH));
                remaining -= BLOCK_LENGTH;
            }

            if(remaining > 0) {
                Pack.intToBigEndian(h8, output, offset + 28, Math.min(remaining, BLOCK_LENGTH));
            }
