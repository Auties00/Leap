    ...
    private static final class S30VerifyDataGenerator extends TlsHandshakeHash {
        ...
        public byte[] finish(TlsEngine engine, TlsMessage.Source source) {
            var mode = engine.selectedMode()
                    .orElseThrow(() -> new TlsSpecificationException("Missing tls mode"));
            var masterSecret = engine.sessionKeys()
                    .orElseThrow(() -> new TlsSpecificationException("Missing tls session keys"))
                    .masterSecretKey()
                    .data();
            var useClientLabel = (mode == TlsEngineMode.CLIENT && source == TlsMessage.Source.LOCAL) || (mode == TlsEngineMode.SERVER && source == TlsMessage.Source.REMOTE);
            if (useClientLabel) {
    ...
    private static final class T10VerifyDataGenerator extends TlsHandshakeHash {
        ...
        public byte[] finish(TlsEngine engine, TlsMessage.Source source) {
            ...
            var masterSecret = engine.sessionKeys()
                    ...
                    .masterSecretKey()
                    .data();
            boolean useClientLabel = (mode == TlsEngineMode.CLIENT && source == TlsMessage.Source.LOCAL) || (mode == TlsEngineMode.SERVER && source == TlsMessage.Source.REMOTE);
