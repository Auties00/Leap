...
final class SM3Hash implements TlsHash {
    ...
    public int digest(byte[] buf, int offset, int length, boolean reset) {
        if (reset) {
            ...
            for(var i : v) {
                BufferHelper.writeBigEndianInt32(i, buf, offset);
            }
    ...
    private void processWord(byte[] in, int inOff) {
        inwords[xOff++] = BufferHelper.readBigEndianInt32(in, inOff);
        if (this.xOff >= 16) {
            processBlock();
        }
    }
    ...
    private void processWord(ByteBuffer in) {
        inwords[xOff++] = BufferHelper.readBigEndianInt32(in);
        if (this.xOff >= 16) {
            processBlock();
        }
    }
