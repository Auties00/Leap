...
public final class SHA384 extends TlsHash {
    ...
    public int digest(byte[] output, int offset, int length, boolean reset) {
        if (reset) {
            ...
            if(remainingLength > 0) {
                Pack.longToBigEndian(h1, output, offset, Math.min(remainingLength, BLOCK_LENGTH));
                remainingLength -= BLOCK_LENGTH;
            }

            if(remainingLength > 0) {
                Pack.longToBigEndian(h2, output, offset + 8, Math.min(remainingLength, BLOCK_LENGTH));
                remainingLength -= BLOCK_LENGTH;
            }

            if(remainingLength > 0) {
                Pack.longToBigEndian(h3, output, offset + 16, Math.min(remainingLength, BLOCK_LENGTH));
                remainingLength -= BLOCK_LENGTH;
            }

            if(remainingLength > 0) {
                Pack.longToBigEndian(h4, output, offset + 24, Math.min(remainingLength, BLOCK_LENGTH));
                remainingLength -= BLOCK_LENGTH;
            }

            if(remainingLength > 0) {
                Pack.longToBigEndian(h5, output, offset + 32, Math.min(remainingLength, BLOCK_LENGTH));
                remainingLength -= BLOCK_LENGTH;
            }

            if(remainingLength > 0) {
                Pack.longToBigEndian(h6, output, offset + 40, Math.min(remainingLength, BLOCK_LENGTH));
            }
