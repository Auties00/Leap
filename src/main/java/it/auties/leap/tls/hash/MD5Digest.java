...
public final class MD5Digest implements TlsHashType {
    ...
    private MD5Digest(MD5Digest other) {
        this.h1 = other.h1;
        this.h2 = other.h2;
        this.h3 = other.h3;
        this.h4 = other.h4;
    ...
    public int digest(byte[] output, int offset, int length, boolean reset) {
        if (reset) {
            ...
            if(remaining > 0) {
                writeLittleEndianInt32(h1, output, offset, Math.min(remaining, BLOCK_LENGTH));
                remaining -= BLOCK_LENGTH;
            }

            if(remaining > 0) {
                writeLittleEndianInt32(h2, output, offset + 4, Math.min(remaining, BLOCK_LENGTH));
                remaining -= BLOCK_LENGTH;
            }

            if(remaining > 0) {
                writeLittleEndianInt32(h3, output, offset + 8, Math.min(remaining, BLOCK_LENGTH));
                remaining -= BLOCK_LENGTH;
            }

            if(remaining > 0) {
                writeLittleEndianInt32(h4, output, offset + 12, Math.min(remaining, BLOCK_LENGTH));
            }
