...
final class CBCWrapper extends TlsCipherWrapper {
    ...
    CBCWrapper(TlsVersion version, TlsCipher cipher, TlsExchangeAuthenticator authenticator, TlsSessionKeys sessionKeys, TlsMode mode, boolean reducedKeySize) {
    ...
    public void decrypt(ContentType contentType, ByteBuffer input, ByteBuffer output, byte[] sequence) {
        switch (version) {
            ...
            case TLS13, DTLS13 -> throw new TlsException("BLOCK ciphers are not allowed in (D)TLSv1.3");
        }
    }
