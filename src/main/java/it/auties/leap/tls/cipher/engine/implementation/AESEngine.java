...
public final class AESEngine extends TlsCipherEngine.Block {
    ...
    public AESEngine(boolean forEncryption, byte[] key) {
    ...
    private void handle128BitsKey(byte[] key, int[][] blocks) {
        var col0 = BufferUtils.readLittleEndianInt32(key, 0);
        blocks[0][0] = col0;

        var col1 = BufferUtils.readLittleEndianInt32(key, 4);
        blocks[0][1] = col1;

        var col2 = BufferUtils.readLittleEndianInt32(key, 8);
        blocks[0][2] = col2;

        var col3 = BufferUtils.readLittleEndianInt32(key, 12);
        blocks[0][3] = col3;
    ...
    private void handle256BitsKey(byte[] key, int[][] blocks) {
        ...
        var col4 = BufferUtils.readLittleEndianInt32(key, 16);
        blocks[1][0] = col4;

        var col5 = BufferUtils.readLittleEndianInt32(key, 20);
        blocks[1][1] = col5;

        var col6 = BufferUtils.readLittleEndianInt32(key, 24);
        blocks[1][2] = col6;

        var col7 = BufferUtils.readLittleEndianInt32(key, 28);
        blocks[1][3] = col7;
    ...
    private void encryptBlock(ByteBuffer input, ByteBuffer output) {
        var c0 = BufferUtils.readLittleEndianInt32(input);
        var c1 = BufferUtils.readLittleEndianInt32(input);
        var c2 = BufferUtils.readLittleEndianInt32(input);
        var c3 = BufferUtils.readLittleEndianInt32(input);

        var t0 = c0 ^ workingKey[0][0];
        var t1 = c1 ^ workingKey[0][1];
        var t2 = c2 ^ workingKey[0][2];

        int r = 1, r0, r1, r2, r3 = c3 ^ workingKey[0][3];
        while (r < rounds - 1) {
            r0 = ROUNDS_CALCULATIONS[t0 & 255] ^ shift(ROUNDS_CALCULATIONS[(t1 >> 8) & 255], 24) ^ shift(ROUNDS_CALCULATIONS[(t2 >> 16) & 255], 16) ^ shift(ROUNDS_CALCULATIONS[(r3 >> 24) & 255], 8) ^ workingKey[r][0];
            r1 = ROUNDS_CALCULATIONS[t1 & 255] ^ shift(ROUNDS_CALCULATIONS[(t2 >> 8) & 255], 24) ^ shift(ROUNDS_CALCULATIONS[(r3 >> 16) & 255], 16) ^ shift(ROUNDS_CALCULATIONS[(t0 >> 24) & 255], 8) ^ workingKey[r][1];
            r2 = ROUNDS_CALCULATIONS[t2 & 255] ^ shift(ROUNDS_CALCULATIONS[(r3 >> 8) & 255], 24) ^ shift(ROUNDS_CALCULATIONS[(t0 >> 16) & 255], 16) ^ shift(ROUNDS_CALCULATIONS[(t1 >> 24) & 255], 8) ^ workingKey[r][2];
            r3 = ROUNDS_CALCULATIONS[r3 & 255] ^ shift(ROUNDS_CALCULATIONS[(t0 >> 8) & 255], 24) ^ shift(ROUNDS_CALCULATIONS[(t1 >> 16) & 255], 16) ^ shift(ROUNDS_CALCULATIONS[(t2 >> 24) & 255], 8) ^ workingKey[r++][3];
            t0 = ROUNDS_CALCULATIONS[r0 & 255] ^ shift(ROUNDS_CALCULATIONS[(r1 >> 8) & 255], 24) ^ shift(ROUNDS_CALCULATIONS[(r2 >> 16) & 255], 16) ^ shift(ROUNDS_CALCULATIONS[(r3 >> 24) & 255], 8) ^ workingKey[r][0];
            t1 = ROUNDS_CALCULATIONS[r1 & 255] ^ shift(ROUNDS_CALCULATIONS[(r2 >> 8) & 255], 24) ^ shift(ROUNDS_CALCULATIONS[(r3 >> 16) & 255], 16) ^ shift(ROUNDS_CALCULATIONS[(r0 >> 24) & 255], 8) ^ workingKey[r][1];
            t2 = ROUNDS_CALCULATIONS[r2 & 255] ^ shift(ROUNDS_CALCULATIONS[(r3 >> 8) & 255], 24) ^ shift(ROUNDS_CALCULATIONS[(r0 >> 16) & 255], 16) ^ shift(ROUNDS_CALCULATIONS[(r1 >> 24) & 255], 8) ^ workingKey[r][2];
            r3 = ROUNDS_CALCULATIONS[r3 & 255] ^ shift(ROUNDS_CALCULATIONS[(r0 >> 8) & 255], 24) ^ shift(ROUNDS_CALCULATIONS[(r1 >> 16) & 255], 16) ^ shift(ROUNDS_CALCULATIONS[(r2 >> 24) & 255], 8) ^ workingKey[r++][3];
        }
        ...
        c0 = (S_BOX[r0 & 255] & 255) ^ ((S_BOX[(r1 >> 8) & 255] & 255) << 8) ^ ((s[(r2 >> 16) & 255] & 255) << 16) ^ (s[(r3 >> 24) & 255] << 24) ^ workingKey[r][0];
        c1 = (s[r1 & 255] & 255) ^ ((S_BOX[(r2 >> 8) & 255] & 255) << 8) ^ ((S_BOX[(r3 >> 16) & 255] & 255) << 16) ^ (s[(r0 >> 24) & 255] << 24) ^ workingKey[r][1];
        ...
        BufferUtils.writeLittleEndianInt32(output, c0);
        BufferUtils.writeLittleEndianInt32(output, c1);
        BufferUtils.writeLittleEndianInt32(output, c2);
        BufferUtils.writeLittleEndianInt32(output, c3);
    }
    ...
    private void decryptBlock(ByteBuffer input, ByteBuffer output) {
        ...
        int t2 = c2 ^ workingKey[rounds][2];

        int r = rounds - 1, r0, r1, r2, r3 = c3 ^ workingKey[rounds][3];
        while (r > 1) {
            r0 = ROUNDS_CALCULATIONS_INVERSE[t0 & 255] ^ shift(ROUNDS_CALCULATIONS_INVERSE[(r3 >> 8) & 255], 24) ^ shift(ROUNDS_CALCULATIONS_INVERSE[(t2 >> 16) & 255], 16) ^ shift(ROUNDS_CALCULATIONS_INVERSE[(t1 >> 24) & 255], 8) ^ workingKey[r][0];
            r1 = ROUNDS_CALCULATIONS_INVERSE[t1 & 255] ^ shift(ROUNDS_CALCULATIONS_INVERSE[(t0 >> 8) & 255], 24) ^ shift(ROUNDS_CALCULATIONS_INVERSE[(r3 >> 16) & 255], 16) ^ shift(ROUNDS_CALCULATIONS_INVERSE[(t2 >> 24) & 255], 8) ^ workingKey[r][1];
            r2 = ROUNDS_CALCULATIONS_INVERSE[t2 & 255] ^ shift(ROUNDS_CALCULATIONS_INVERSE[(t1 >> 8) & 255], 24) ^ shift(ROUNDS_CALCULATIONS_INVERSE[(t0 >> 16) & 255], 16) ^ shift(ROUNDS_CALCULATIONS_INVERSE[(r3 >> 24) & 255], 8) ^ workingKey[r][2];
            r3 = ROUNDS_CALCULATIONS_INVERSE[r3 & 255] ^ shift(ROUNDS_CALCULATIONS_INVERSE[(t2 >> 8) & 255], 24) ^ shift(ROUNDS_CALCULATIONS_INVERSE[(t1 >> 16) & 255], 16) ^ shift(ROUNDS_CALCULATIONS_INVERSE[(t0 >> 24) & 255], 8) ^ workingKey[r--][3];
            t0 = ROUNDS_CALCULATIONS_INVERSE[r0 & 255] ^ shift(ROUNDS_CALCULATIONS_INVERSE[(r3 >> 8) & 255], 24) ^ shift(ROUNDS_CALCULATIONS_INVERSE[(r2 >> 16) & 255], 16) ^ shift(ROUNDS_CALCULATIONS_INVERSE[(r1 >> 24) & 255], 8) ^ workingKey[r][0];
            t1 = ROUNDS_CALCULATIONS_INVERSE[r1 & 255] ^ shift(ROUNDS_CALCULATIONS_INVERSE[(r0 >> 8) & 255], 24) ^ shift(ROUNDS_CALCULATIONS_INVERSE[(r3 >> 16) & 255], 16) ^ shift(ROUNDS_CALCULATIONS_INVERSE[(r2 >> 24) & 255], 8) ^ workingKey[r][1];
            t2 = ROUNDS_CALCULATIONS_INVERSE[r2 & 255] ^ shift(ROUNDS_CALCULATIONS_INVERSE[(r1 >> 8) & 255], 24) ^ shift(ROUNDS_CALCULATIONS_INVERSE[(r0 >> 16) & 255], 16) ^ shift(ROUNDS_CALCULATIONS_INVERSE[(r3 >> 24) & 255], 8) ^ workingKey[r][2];
            r3 = ROUNDS_CALCULATIONS_INVERSE[r3 & 255] ^ shift(ROUNDS_CALCULATIONS_INVERSE[(r2 >> 8) & 255], 24) ^ shift(ROUNDS_CALCULATIONS_INVERSE[(r1 >> 16) & 255], 16) ^ shift(ROUNDS_CALCULATIONS_INVERSE[(r0 >> 24) & 255], 8) ^ workingKey[r--][3];
        }
        ...
        c0 = (S_BOX_INVERSE[r0 & 255] & 255) ^ ((s[(r3 >> 8) & 255] & 255) << 8) ^ ((s[(r2 >> 16) & 255] & 255) << 16) ^ (S_BOX_INVERSE[(r1 >> 24) & 255] << 24) ^ workingKey[0][0];
        c1 = (s[r1 & 255] & 255) ^ ((s[(r0 >> 8) & 255] & 255) << 8) ^ ((S_BOX_INVERSE[(r3 >> 16) & 255] & 255) << 16) ^ (s[(r2 >> 24) & 255] << 24) ^ workingKey[0][1];
