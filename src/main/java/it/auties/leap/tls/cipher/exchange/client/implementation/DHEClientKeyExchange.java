...
import it.auties.leap.tls.cipher.exchange.server.implementation.DHEServerKeyExchange;
...
public final class DHEClientKeyExchange implements TlsClientKeyExchange {
    private static final TlsClientKeyExchangeFactory FACTORY = engine -> engine.localKeyPair()
            .map(keyPair -> new DHEClientKeyExchange(keyPair.getPublic()))
            .orElseThrow(() -> new TlsException("Missing keypair"));

    private final byte[] y;
    private final DHPublicKey publicKey;

    public DHEClientKeyExchange(PublicKey publicKey) {
    ...
    public DHEClientKeyExchange(ByteBuffer buffer, byte[] p, byte[] g) {
        this(buffer, KeyUtils.fromUnsignedLittleEndianBytes(p), fromUnsignedLittleEndianBytes(g));
    }

    public DHEClientKeyExchange(ByteBuffer buffer, BigInteger p, BigInteger g) {
    ...
    public TlsClientKeyExchange decodeLocal(ByteBuffer buffer) {
        return new DHEClientKeyExchange(buffer, publicKey.getParams().getP(), publicKey.getParams().getG());
    }
    ...
    public byte[] generatePreMasterSecret(PrivateKey localPrivateKey, PublicKey remoteCertificatePublicKey, TlsKeyExchange remoteKeyExchange) {
        if (!(remoteKeyExchange instanceof DHEServerKeyExchange serverKeyExchange)) {
