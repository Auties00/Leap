...
public final class RC2Engine extends TlsCipherEngine.Block {
    ...
    private int[] generateWorkingKey(byte[] key, int bits) {
        ...
        if (len < 128) {
            ...
            do {
                x = PI_TABLE[(x + xKey[index++]) & 255] & 0xff;
                xKey[len++] = x;
            } while (len < 128);
        }

        len = (bits + 7) >> 3;
        x = PI_TABLE[xKey[128 - len] & (255 >> (7 & -bits))] & 0xff;
        xKey[128 - len] = x;

        for (var i = 128 - len - 1; i >= 0; i--) {
            x = PI_TABLE[x ^ xKey[i + len]] & 0xff;
            xKey[i] = x;
        }
    ...
    private void encryptBlock(ByteBuffer input, ByteBuffer output) {
        var x10 = BufferUtils.readLittleEndianInt16(input);
        var x32 = BufferUtils.readLittleEndianInt16(input);
        var x54 = BufferUtils.readLittleEndianInt16(input);
        var x76 = BufferUtils.readLittleEndianInt16(input);
        ...
        BufferUtils.writeLittleEndianInt16(output, x10);
        BufferUtils.writeLittleEndianInt16(output, x32);
        BufferUtils.writeLittleEndianInt16(output, x54);
        BufferUtils.writeLittleEndianInt16(output, x76);
    }
