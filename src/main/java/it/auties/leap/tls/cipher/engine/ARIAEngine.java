...
public final class ARIAEngine extends TlsCipherEngine.Block {
    ...
    private byte[][] keySchedule(byte[] key) {
        ...
        System.arraycopy(key, 16, KR, 0, keyLen - 16);
        ...
        var rks = new byte[numRounds + 1][16];
    ...
    private void reverseKeys(byte[][] keys) {
    ...
    private void FE(byte[] D, byte[] RK) {
        xor(D, RK);
        SL2(D);
        A(D);
    }

    private void FO(byte[] D, byte[] RK) {
        xor(D, RK);
        SL1(D);
        A(D);
    }
    
    private void SL1(byte[] z) {
        z[0] = SB1(z[0]);
        z[1] = SB2(z[1]);
        z[2] = SB3(z[2]);
        z[3] = SB4(z[3]);
    ...
    private byte SB1(byte x) {
        return S_BOX_1[x & 0xFF];
    }

    private byte SB2(byte x) {
        return S_BOX_2[x & 0xFF];
    }

    private byte SB3(byte x) {
        return S_BOX_3[x & 0xFF];
    }

    private byte SB4(byte x) {
        return S_BOX_4[x & 0xFF];
    }

    private void xor(byte[] z, byte[] x) {
        for (int i = 0; i < 16; ++i) {
            z[i] ^= x[i];
        }
    }
