...
public sealed abstract class CamelliaEngine extends TlsCipherEngine.Block {
    ...
    private static final TlsCipherEngineFactory FACTORY_128 = new TlsCipherEngineFactory.Block() {
        ...
        public TlsCipherEngine newCipherEngine(boolean forEncryption, byte[] key) {
            return new Camellia128Engine(forEncryption, key);
        }
    ...
    private static final TlsCipherEngineFactory FACTORY_256 = new TlsCipherEngineFactory.Block() {
        ...
        @Override
        public TlsCipherEngine newCipherEngine(boolean forEncryption, byte[] key) {
            return new Camellia256Engine(forEncryption, key);
        }
    ...
    private CamelliaEngine(boolean forEncryption, byte[] key) {
        super(forEncryption);
        this.subkey = new int[96];
        this.kw = new int[8];
        this.ke = new int[12];
        initKey(forEncryption, key);
    }
    ...
    abstract void initKey(boolean forEncryption, byte[] key);
    ...
    void roldq(int rot, int[] ki, int ioff, int[] ko, int ooff) {
    ...
    void decroldq(int rot, int[] ki, int ioff, int[] ko, int ooff) {
    ...
    void roldqo32(int rot, int[] ki, int ioff, int[] ko, int ooff) {
    ...
    void decroldqo32(int rot, int[] ki, int ioff, int[] ko, int ooff) {
    ...
    void camelliaF2(int[] s, int[] skey, int keyoff) {
        int t1, t2, u, v;

        t1 = s[0] ^ skey[keyoff];
        u = S_BOX_4[t1 & 0xff];
        u ^= S_BOX_3[(t1 >>> 8) & 0xff];
        u ^= S_BOX_2[(t1 >>> 16) & 0xff];
        u ^= S_BOX_1[(t1 >>> 24) & 0xff];
        t2 = s[1] ^ skey[1 + keyoff];
        v = S_BOX_1[t2 & 0xff];
        v ^= S_BOX_4[(t2 >>> 8) & 0xff];
        v ^= S_BOX_3[(t2 >>> 16) & 0xff];
        v ^= S_BOX_2[(t2 >>> 24) & 0xff];

        s[2] ^= u ^ v;
        s[3] ^= u ^ v ^ rightRotate(u);
        ...
        s[0] ^= u ^ v;
        s[1] ^= u ^ v ^ rightRotate(u);
    }

    void camelliaFLs(int[] s, int[] fkey, int keyoff) {
        s[1] ^= leftRotate(s[0] & fkey[keyoff]);
    ...
    private static final class Camellia128Engine extends CamelliaEngine {
        private Camellia128Engine(boolean forEncryption, byte[] key) {
            super(forEncryption, key);
        }

        @Override
        void initKey(boolean forEncryption, byte[] key) {
            var k = new int[8];
            var ka = new int[4];
            var kb = new int[4];
            var t = new int[4];

            k[0] = readBigEndianInt32(key, 0);
            k[1] = readBigEndianInt32(key, 4);
            k[2] = readBigEndianInt32(key, 8);
            k[3] = readBigEndianInt32(key, 12);
            k[4] = k[5] = k[6] = k[7] = 0;
        ...
        public void cipher(ByteBuffer input, ByteBuffer output) {
            var state = new int[4];
            for (int i = 0; i < 4; i++) {
                state[i] = readBigEndianInt32(input) ^ kw[i];
            }
            ...
            writeBigEndianInt32(output, state[2]);
            writeBigEndianInt32(output, state[3]);
            writeBigEndianInt32(output, state[0]);
            writeBigEndianInt32(output, state[1]);
        }
    }

    private static final class Camellia256Engine extends CamelliaEngine {
        private Camellia256Engine(boolean forEncryption, byte[] key) {
            super(forEncryption, key);
        }
        ...
        void initKey(boolean forEncryption, byte[] key) {
            ...
            k[2] = readBigEndianInt32(key, 8);
            k[3] = readBigEndianInt32(key, 12);
            k[4] = readBigEndianInt32(key, 16);
            k[5] = readBigEndianInt32(key, 20);
            k[6] = readBigEndianInt32(key, 24);
            k[7] = readBigEndianInt32(key, 28);
