...
public final class CamelliaEngine extends TlsCipherEngine.Block {
    ...
    private static final TlsCipherEngineFactory FACTORY = CamelliaEngine::new;

    private final int[] subkey;
    private final int[] kw;
    private final int[] ke;
    public CamelliaEngine(boolean forEncryption, byte[] key) {
    ...
    private void read128BitKey(byte[] key, int[] k) {
        k[0] = BufferUtils.readBigEndianInt32(key, 0);
        k[1] = BufferUtils.readBigEndianInt32(key, 4);
        k[2] = BufferUtils.readBigEndianInt32(key, 8);
        k[3] = BufferUtils.readBigEndianInt32(key, 12);
        k[4] = k[5] = k[6] = k[7] = 0;
    }
    ...
    private void read256BitKey(byte[] key, int[] k) {
        ...
        k[2] = BufferUtils.readBigEndianInt32(key, 8);
        k[3] = BufferUtils.readBigEndianInt32(key, 12);
        k[4] = BufferUtils.readBigEndianInt32(key, 16);
        k[5] = BufferUtils.readBigEndianInt32(key, 20);
        k[6] = BufferUtils.readBigEndianInt32(key, 24);
        k[7] = BufferUtils.readBigEndianInt32(key, 28);
    }
    ...
    private void processBlock128(ByteBuffer input, ByteBuffer output) {
        var state = new int[4];
        for (int i = 0; i < 4; i++) {
            state[i] = BufferUtils.readBigEndianInt32(input) ^ kw[i];
        }
        ...
        BufferUtils.writeBigEndianInt32(output, state[2]);
        BufferUtils.writeBigEndianInt32(output, state[3]);
        BufferUtils.writeBigEndianInt32(output, state[0]);
        BufferUtils.writeBigEndianInt32(output, state[1]);
    }
