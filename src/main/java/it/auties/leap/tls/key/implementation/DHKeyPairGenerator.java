package it.auties.leap.tls.key.implementation;

import it.auties.leap.tls.exception.TlsException;
import it.auties.leap.tls.key.TlsKeyPairGenerator;
import it.auties.leap.tls.version.TlsVersion;

import javax.crypto.spec.DHParameterSpec;
import java.math.BigInteger;
import java.security.GeneralSecurityException;
import java.security.KeyPair;
import java.security.KeyPairGenerator;

public final class DHKeyPairGenerator implements TlsKeyPairGenerator {
    private static final BigInteger P2048 = new BigInteger("", 16);
    private static final BigInteger P3072 = new BigInteger("", 16);
    private static final BigInteger P4096 = new BigInteger("", 16);
    private static final BigInteger P6144 = new BigInteger("", 16);
    private static final BigInteger P8192 = new BigInteger("", 16);

    private static final DHKeyPairGenerator FFDHE2048 = new DHKeyPairGenerator(new DHParameterSpec(P2048, BigInteger.TWO));
    private static final DHKeyPairGenerator FFDHE3072 = new DHKeyPairGenerator(new DHParameterSpec(P3072, BigInteger.TWO));
    private static final DHKeyPairGenerator FFDHE4096 = new DHKeyPairGenerator(new DHParameterSpec(P4096, BigInteger.TWO));
    private static final DHKeyPairGenerator FFDHE6144 = new DHKeyPairGenerator(new DHParameterSpec(P6144, BigInteger.TWO));
    private static final DHKeyPairGenerator FFDHE8192 = new DHKeyPairGenerator(new DHParameterSpec(P8192, BigInteger.TWO));

    private final DHParameterSpec spec;

    private DHKeyPairGenerator(DHParameterSpec spec) {
        this.spec = spec;
    }

    public static DHKeyPairGenerator ffdhe2048() {
        return FFDHE2048;
    }

    public static DHKeyPairGenerator ffdhe3072() {
        return FFDHE3072;
    }

    public static DHKeyPairGenerator ffdhe4096() {
        return FFDHE4096;
    }

    public static DHKeyPairGenerator ffdhe6144() {
        return FFDHE6144;
    }

    public static DHKeyPairGenerator ffdhe8192() {
        return FFDHE8192;
    }

    @Override
    public KeyPair generate(TlsVersion version) {
        try {
            var keyPairGenerator = KeyPairGenerator.getInstance("DH");
            keyPairGenerator.initialize(spec);
            return keyPairGenerator.generateKeyPair();
        } catch (GeneralSecurityException exception) {
            throw new TlsException("Cannot generate DHE keypair", exception);
        }
    }
}
