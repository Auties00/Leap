...
import it.auties.leap.tls.crypto.hash.TlsPRF;
import it.auties.leap.tls.crypto.hash.TlsHash;
...
import static it.auties.leap.tls.crypto.key.TlsKeyConstants.*;
...
public record TlsSessionKeys(
    ...
    public static TlsSessionKeys of(
        ...
        var macLength = cipher.hmac()
                .length();
        var expandedKeyLength = cipher.type()
                .expandedKeyLength()
                .orElse(-1);
        var keyLength = cipher.type()
                .cipherKeyLength();
        var ivLength = cipher.type().ivLength() - DYNAMIC_IV_LENGTH;
        ...
        if (expandedKeyLength == -1) {
            var clientIv = TlsBuffer.readBytes(keyBlock, ivLength);
            var serverIv = TlsBuffer.readBytes(keyBlock, ivLength);
            return new TlsSessionKeys(
                    masterSecret,
                    clientMacKey,
                    serverMacKey,
