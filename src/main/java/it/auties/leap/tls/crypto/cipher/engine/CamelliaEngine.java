...
final class CamelliaEngine extends TlsCipherEngine.Block {
    ...
    private static void handle128BitsKey(byte[] key, int[] k) {
        k[0] = TlsBuffer.readBigEndianInt32(key, 0);
        k[1] = TlsBuffer.readBigEndianInt32(key, 4);
        k[2] = TlsBuffer.readBigEndianInt32(key, 8);
        k[3] = TlsBuffer.readBigEndianInt32(key, 12);
        k[4] = k[5] = k[6] = k[7] = 0;
    }
    ...
    private static void handle192BitsKey(byte[] key, int[] k) {
        ...
        k[2] = TlsBuffer.readBigEndianInt32(key, 8);
        k[3] = TlsBuffer.readBigEndianInt32(key, 12);
        k[4] = TlsBuffer.readBigEndianInt32(key, 16);
        k[5] = TlsBuffer.readBigEndianInt32(key, 20);
        k[6] = ~k[4];
        k[7] = ~k[5];
    }
    ...
    private static void handle256BitsKey(byte[] key, int[] k) {
        ...
        k[4] = TlsBuffer.readBigEndianInt32(key, 16);
        k[5] = TlsBuffer.readBigEndianInt32(key, 20);
        k[6] = TlsBuffer.readBigEndianInt32(key, 24);
        k[7] = TlsBuffer.readBigEndianInt32(key, 28);
    }
    ...
    private void processBlock128(ByteBuffer input, ByteBuffer output) {
        var state = new int[4];
        for (int i = 0; i < 4; i++) {
            state[i] = TlsBuffer.readBigEndianInt32(input) ^ kw[i];
        }
        ...
        TlsBuffer.writeBigEndianInt32(output, state[2]);
        TlsBuffer.writeBigEndianInt32(output, state[3]);
        TlsBuffer.writeBigEndianInt32(output, state[0]);
        TlsBuffer.writeBigEndianInt32(output, state[1]);
    }
