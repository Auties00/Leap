...
final class IDEAEngine extends TlsCipherEngine.Block {
    ...
    private int[] expandKey(byte[] userKey) {
        ...
        for (var i = 0; i < 8; i++) {
            key[i] = TlsBuffer.readBigEndianInt16(userKey, i * 2);
        }
    ...
    public void process(ByteBuffer input, ByteBuffer output) {
        var keyOff = 0;
        var x0 = TlsBuffer.readBigEndianInt16(input);
        var x1 = TlsBuffer.readBigEndianInt16(input);
        var x2 = TlsBuffer.readBigEndianInt16(input);
        var x3 = TlsBuffer.readBigEndianInt16(input);
        ...
        TlsBuffer.writeBigEndianInt16(output, mul(x0, workingKey[keyOff++]));
        TlsBuffer.writeBigEndianInt16(output, x2 + workingKey[keyOff++]);
        TlsBuffer.writeBigEndianInt16(output, x1 + workingKey[keyOff++]);
        TlsBuffer.writeBigEndianInt16(output, mul(x3, workingKey[keyOff]));
    }
