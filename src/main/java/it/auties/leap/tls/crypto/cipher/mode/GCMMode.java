...
final class GCMMode implements TlsCipherMode.Block, TlsCipherMode.AEAD {
    ...
    GCMMode(TlsCipherEngine.Block c, GCMMultiplier m, byte[] iv, byte[] aad) {
        ...
        if (iv.length == 12) {
            ...
            TlsBuffer.longToBigEndian((long) iv.length * 8, X, 8);
            gHASHBlock(J0, X);
        }
    ...
    @Override
    public int update(byte[] in, int inOff, byte[] out, int outOff) {
        checkStatus();
    ...
    public int doFinal(byte[] out, int outOff) {
        ...
        // Final gHASH
        byte[] X = new byte[BLOCK_SIZE];
        TlsBuffer.longToBigEndian(atLength * 8, X, 0);
        TlsBuffer.longToBigEndian(totalLength * 8, X, 8);
    ...
    public sealed static interface GCMMultiplier {
        ...
        final class Tables4kGCMMultiplier implements GCMMultiplier {
            ...
            public void multiplyH(byte[] x) {
                ...
                TlsBuffer.longToBigEndian(z0, x, 0);
                TlsBuffer.longToBigEndian(z1, x, 8);
            }
        }
    ...
    public static final class GCMUtil {
        ...
        public static void asBytes(long[] x, byte[] z) {
            TlsBuffer.longToBigEndian(x, 0, SIZE_Long, z, 0);
        }

        public static long[] asLongs(byte[] x) {
            long[] z = new long[SIZE_Long];
            TlsBuffer.bigEndianToLong(x, 0, z, 0, SIZE_Long);
            return z;
        }
        ...
        static void multiply(byte[] x, long[] y) {
            ...
            long x0 = TlsBuffer.bigEndianToLong(x, 0);
            long x1 = TlsBuffer.bigEndianToLong(x, 8);
            long y0 = y[0], y1 = y[1];
