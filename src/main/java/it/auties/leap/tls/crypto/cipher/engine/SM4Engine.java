...
final class SM4Engine extends TlsCipherEngine.Block {
    ...
    private int[] expandKey(boolean forEncryption, byte[] key) {
        ...
        mk[0] = TlsBuffer.readBigEndianInt32(key, 0);
        mk[1] = TlsBuffer.readBigEndianInt32(key, 4);
        mk[2] = TlsBuffer.readBigEndianInt32(key, 8);
        mk[3] = TlsBuffer.readBigEndianInt32(key, 12);
    ...
    public void process(ByteBuffer input, ByteBuffer output) {
        x[0] = TlsBuffer.readBigEndianInt32(input);
        x[1] = TlsBuffer.readBigEndianInt32(input);
        x[2] = TlsBuffer.readBigEndianInt32(input);
        x[3] = TlsBuffer.readBigEndianInt32(input);
        ...
        TlsBuffer.writeBigEndianInt32(output, x[3]);
        TlsBuffer.writeBigEndianInt32(output, x[2]);
        TlsBuffer.writeBigEndianInt32(output, x[1]);
        TlsBuffer.writeBigEndianInt32(output, x[0]);
    }
    ...
    private int tau(int A) {
        return ((S_BOX[(A >> 24) & 0xff] & 0xff) << 24)
                | ((S_BOX[(A >> 16) & 0xff] & 0xff) << 16)
                | ((S_BOX[(A >> 8) & 0xff] & 0xff) << 8)
                | (S_BOX[A & 0xff] & 0xff);
    }
